    ...
    useEffect(() => {
        ...
        if (user) {
            ...
            if (userDoc.exists()) { // Check if the document exists
                currentUser = userDoc.data(); // Access data from the document snapshot 
            } 
        }
        
        console.log('cur/instructor page', currentUser)

        let coursesSnapshot
        if (currentUser?.role.toLowerCase() === 'instructor') {
            const instructorCoursesQuery = query(coursesCollection,where('email', '==', currentUser.email));
